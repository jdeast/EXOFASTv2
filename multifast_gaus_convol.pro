...
function multifast_gaus_convol,x,y,sig,nsigma=nsigma

on_error,2
c=check_math(0,1)
if n_params() ne 3 then message,'usage: result=multifast_gaus_convol(x,y,sig)'
...
;if ((conv le 0d0) or (~finite(conv))) then return, !values.d_nan
...
gauscon = convol(y,gaus,/center)/total(gaus)       ; do convolution.
;if ((gauscon EQ !null) or (n_elements(gauscon) eq 0)) then gauscon = !values.d_nan
return, gauscon
end
