pro upperlimit, filename, names=names, sigma=sigma

if n_elements(sigma) eq 0 then sigma = 3d0
if n_elements(names) eq 0 then $
   names = ['M_P','K','\rho_P','logg_P','M_P\sin i','M_P/M_*']
                        ...
                        for m=0L, n_elements((*(mcmcss.(i)[j].(k))).(l))-1 do begin
                           if tag_exist((*(mcmcss.(i)[j].(k))).(l)[m],'latex') then begin
                              if (*(mcmcss.(i)[j].(k))).(l)[m].latex eq names[n] then begin
                                 ...
                                 upperlimit = (par[sort(par)])[round(sigmapercent*nsteps)]
                                 print, strtrim(sigma,2) + ' sigma upper limit on ' + names[n] + ',' + strtrim(j,2) + ' (' + units + ') ' + strtrim(upperlimit,2)
                                 found = 1
                              endif
                           endif
                        endfor
               ...
               if n_tags(mcmcss.(i)[j].(k)) ne 0 then begin
                  if tag_exist(mcmcss.(i)[j].(k),'latex') then begin
                     if mcmcss.(i)[j].(k).latex eq names[n] then begin
                        ...
                        nsteps = n_elements(par)
                        upperlimit = (par[sort(par)])[round(sigmapercent*nsteps)]
                        print, strtrim(sigma,2) + ' sigma upper limit on ' + names[n] + ',' + strtrim(j,2) + unitstr + strtrim(upperlimit,2)      
                        found = 1
                     endif
                  endif
               endif
