...
; INPUTS:
;
;    PARS - An array of parameters that describes the model (this
;           should be generated by structure2pars)
...
if step2pars(ss) eq -1 then begin
...
;if ss.debug then print, ss.(priors[0,i])[priors[1,i]].(priors[2,i]).value, $
;          ss.(priors[0,i])[priors[1,i]].(priors[2,i]).prior, $
;          ss.(priors[0,i])[priors[1,i]].(priors[2,i]).priorwidth, $
;          ss.(priors[0,i])[priors[1,i]].(priors[2,i]).label
endfor


;; print all the parameters and the chi^2
if ss.debug then print, pars, chi2, format='(' + strtrim(n_elements(pars)+1,2) + '(f0.6,x))'
