...
;-
pro exofast_legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
    ...
    OUTLINE_COLOR = outline_color, FONT = font, USEPLOTSYM=useplotsym
;
;       =====>> HELP
;
compile_opt idl2
...
if vertical then begin                          ; CALC OFFSET FOR TEXT START
  for i = 0,n-1 do begin
    if (psym[i] eq 0) and (vectorfont[i] eq '') and ~keyword_set(useplotsym) then num = (number + 1) > 3 else num = number
    if psym[i] lt 0 and ~keyword_set(useplotsym) then num = number > 2       ; TO SHOW CONNECTING LINE
    if psym[i] eq 0 and ~keyword_set(useplotsym) then expand = 1 else expand = 2
