...
; 
;-
function exofast_gettt, mcmcss, epochs, secondary=secondary, tol=tol, filename=filename, filebase=filebase
...
ttn = dblarr(mcmcss.nsteps,mcmcss.nplanets,mcmcss.ntran) + !values.d_nan
bn = dblarr(mcmcss.nsteps,mcmcss.nplanets,mcmcss.ntran) + !values.d_nan
depthn = dblarr(mcmcss.nsteps,mcmcss.nplanets,mcmcss.ntran) + !values.d_nan
epochs = lonarr(mcmcss.nplanets,mcmcss.ntran)

;; secondary
tsn = dblarr(mcmcss.nsteps,mcmcss.nplanets,mcmcss.ntran) + !values.d_nan
bsn = dblarr(mcmcss.nsteps,mcmcss.nplanets,mcmcss.ntran) + !values.d_nan
epochs2 = lonarr(mcmcss.nplanets,mcmcss.ntran)

for i=0L, mcmcss.nsteps-1 do begin
   ...
   for k=0L, mcmcss.ntran-1 do begin
      ...
      for j=0L, mcmcss.nplanets-1 do begin
         ...
         tp = mcmcss.planet[j].tp.value[i] + mcmcss.transit[k].ttv.value[i]      
         period = mcmcss.planet[j].period.value[i]
         e = mcmcss.planet[j].e.value[i]
         omega = mcmcss.planet[j].omega.value[i]
         t14 = mcmcss.planet[j].t14.value[i]
         t14s = mcmcss.planet[j].t14s.value[i]
         inc = mcmcss.planet[j].i.value[i] + mcmcss.transit[k].tiv.value[i]
         ar = mcmcss.planet[j].ar.value[i]
         q = mcmcss.planet[j].q.value[i]
         p = mcmcss.planet[j].p.value[i]+mcmcss.transit[k].tdeltav.value[i]
         ...
         if i eq mcmcss.nsteps/2 then begin
            epochs[*,k] = epoch
            epochs2[*,k] = epoch2
         endif
         ...
         if maxbjd gt (thistc-t14/2d0) and minbjd lt (thistc+t14/2d0) then begin
            ...
            u1 = mcmcss.band[mcmcss.transit[k].bandndx].u1.value[i]
            u2 = mcmcss.band[mcmcss.transit[k].bandndx].u2.value[i]
         ...
         if maxbjd gt (thists-t14s/2d0) and minbjd lt (thists-t14s/2d0) then begin
        
            ;; find the minimum projected separation during the transit
            ;; assumes time of conjunction is the time of transit to
            ;; within the duration of the transit 
            ;; *** Might not be true if large bodies are in the system ***
            minbjd = (ts + period*epoch2 - t14s)
            maxbjd = (ts + period*epoch2 + t14s)
            tsn[i,j,k] = goldenratio('calcb',minbjd,maxbjd)
   ...
   if ((i+1d0) mod fix(mcmcss.nsteps/10d0)) eq 0 then printandlog, strtrim(round((i+1d0)/mcmcss.nsteps*100),2) + '% done computing transit times', logname

endfor

;; nsteps x nchains x nplanets x ntransits array
ttn = reform(ttn,mcmcss.nsteps/mcmcss.nchains,mcmcss.nchains,mcmcss.nplanets,mcmcss.ntran) 
bn = reform(bn,mcmcss.nsteps/mcmcss.nchains,mcmcss.nchains,mcmcss.nplanets,mcmcss.ntran) 
depthn = reform(depthn,mcmcss.nsteps/mcmcss.nchains,mcmcss.nchains,mcmcss.nplanets,mcmcss.ntran) 
medianttn = [-1]
medianbn = [-1]
mediandepthn = [-1]
...
printf, texlun, '\tablehead{\colhead{Transit} & \colhead{Planet} & \colhead{Epoch} & \colhead{$T_T$} & \colhead{$b$} & \colhead{Depth}}'
...
for j=0L, mcmcss.nplanets-1 do begin
   for k=0L, mcmcss.ntran-1 do begin
      ...
      if ngood gt 0 then begin
         ...
         printf, csvlun, mcmcss.transit[k].label, planetname[j], strtrim(epochs[j,k],2), strtrim(thistt,2), strtrim(tthi,2), strtrim(ttlo,2), strtrim(thisb,2), strtrim(bhi,2), strtrim(blo,2), strtrim(thisdepth,2), strtrim(depthhi,2), strtrim(depthlo,2), format='(a,",",a1,",",a,9(",",a))'
         ...
         printf, texlun, mcmcss.transit[k].label, planetname[j], strtrim(epochs[j,k],2), tt, b, depth, format='(a," & ",a1," & ",a,3(" & ",a),"\\")'
      endif
   endfor
endfor
