pro fittess, debug=debug, verbose=verbose, dothisticid=dothisticid, update=update
...
;readcol, summaryfile, src, ticids, toiid, disposition, ra, dec, tmag, tc, period, duration, depth, sectors, format='a,a,a,a,d,d,d,d,d,d,d,d'
;p = sqrt(depth/1d6)

;tic = read_csv('toi-ete6-2017-12-10.P.TICdata.csv',header=tagnames, n_table_header=4)
...
;   if tic.(srcndx)[shuffle[i]] ne 'qlp' then stop ;continue
   ...
   fitsfiles = file_search(dir + 'tess*' + ticid + '*_lc.fits', count=nfiles)
   if nfiles eq 0 then fitsfiles = file_search(dir + '*' + ticid + '*llc.fits', count=nfiles)
   if nfiles eq 0 then begin
      print, 'No light curves for ' + ticid + ' (' + tic.(srcndx)[shuffle[i]] + ')'
      continue
   endif

;; old file names manually downloaded from MAST
;   fitsfiles = file_search(dir + 'hlsp*' + ticid + '*_lc.fits', count=nfiles)
;   if nfiles eq 0 then begin
;      print, 'No light curves for ' + ticid
;      continue
;   endif
